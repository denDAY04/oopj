...
public class Main
{
    ...
    public void Main()
    {
        // Create objects
          ...
          *created objects, so it can call methoeds in these.*/
         Ser = new Service(UserInt,Stat,cTickets,sBasket);

         // Variabels:  
         int hardwareID=0;
    
         while (cTickets.getArray().isEmpty())
         /*sets up the machine the first time it is run, and aslong as there 
          *are no tickets created.*/
         hardwareID = Ser.setup(); 
