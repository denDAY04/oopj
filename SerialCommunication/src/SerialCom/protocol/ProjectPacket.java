...
/**
 * The <code>ProjectPacket</code> class represents a data packet.
...
public final class ProjectPacket extends Packet {
    ...
    /**
     * Constructs a new <code>ProjectPacket</code> based upon the source, destination,
     ...
     */
    public ProjectPacket(String source, String destination, String commandStatus, String data) {
     ...
     * The byte array should contain data corresponding to all the <code>ProjectPacket</code>
     * fields. The byte array is parsed and the <code>ProjectPacket</code> constructed.
     * @param bytePacket the byte array containing the data for all the <code>ProjectPacket</code> fields.
     */
    ...
    /**
     * Populate the byte array representing the <code>ProjectPacket</code> contents
     ...
     */
    @Override
    public void generateChecksum() {  // needs to replicate the way the package is put together, datalength needs to be padded
        // needs to check against contest of pecket untill but not including checksum
        //String source, String destination, String commandStatus, String data) {
        //super(source, destination, commandStatus, data
        // byte[] b = string.;
        String paddedDataLength = String.format("%04d", this.getDataLength()); // pad data length to 4 with leeding zeros
   ...
   sum ^= b;     // xor of the sum and the current byte element 
     ...
     * <code>ProjectPacket</code> data frame by comparing the number of bytes in the
     ...
     * for the concrete strategy class <code>ProjectPacket</code> in the
