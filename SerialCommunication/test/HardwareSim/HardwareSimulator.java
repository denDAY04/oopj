 ...
 * The <code>HardwareSimulator</code> class simulates the RFID Reader.
 *
 * @version 16/02/10
 * @author ibr
 */
public class HardwareSimulator implements FrameEventListener {
     ...
     */
    public synchronized void sendRequest(String command, String data) {
    ...
    public synchronized void sendRequest(String command, String data,String destination) {
        source = "12";
        System.err.println("Simulator sendRFIDRequest");
        ProjectPacket packet = new ProjectPacket(source, destination, command, data);
        transmitter.transmit(packet.getBytes());
    ...
    private void processRequest(String command) {
        ...
        if (command.equals("VC")) {   // revieves customer data
            sendRequest("RA", "Receive Acknowledged");  // skips checksum check and acknoleges recieved.
            System.err.println("Simulator processRequest: RA Receive Acknowledged sent");
        }
        else if (command.equals("12")) {
            sendRequest("03", "AcceptAck");
